...
class TestInicioFlask(unittest.TestCase):
    def setUp(self):
        # en vez del constructor en las clases comun y corrientes, en los test se usa el metodo setUp 
        # que servira para configurar todas los atributos y demas cosas que vayamos a utilizar en los 
        # escenarios de test de esta clase (metodos)
        self.nombre = 'Joseph'
        # inicia mi aplicacion de flask usando un cliente de test, aceptara peticiones http para probar
        #  los endpoints y toda la aplicacion en general, esto levantara los modelos y hara la conexion 
        # a la bd entre otras cosas
    ...
    def testNombre(self):
        self.assertEqual(self.nombre, 'Josleph')
...
class TestYo(unittest.TestCase):
    ...
    def testPerfil(self):
        respuesta = self.aplicacion_flask.get('/user', headers={'Authorization': 'Bearer {}'.format(self.token)})
        self.assertEqual(respuesta.status_code, 200)
